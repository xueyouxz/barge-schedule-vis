...
function statusToType(currStatus: string, nextStatus: string, samePort: boolean): GanttEventType | null {
...
export function buildBargeCargoGanttData(
  ...
  infos.forEach((barge, index) => {
    ...
    const sortedTrace = [...barge.trace].sort((a, b) => toDate(a.time).getTime() - toDate(b.time).getTime());
    ...
    const maxTeu =
      record?.max_teu ??
      Math.max(
        ...summaries.map((s) => s.num_onboard),
        ...events.map((ev) => ev.teu ?? 0),
        1,
      );
