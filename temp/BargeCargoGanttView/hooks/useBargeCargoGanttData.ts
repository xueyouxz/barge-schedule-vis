 ...
 */
function parseRouteLegs(route?: string): Array<{ fromPort: string; toPort: string; bargeId: number }> {
  if (!route) return [];
...
function buildEtdMarksFromContainerRows(rows: ContainerRecordRow[], dataset: GanttDataset): GanttDataset['etdMarks'] {
...
export function useBargeCargoGanttData(infoPath?: string, recordsPath?: string, containerRecordsPath?: string) {
